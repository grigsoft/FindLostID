...
CFolderDialog::CFolderDialog(LPCTSTR lpszFolderName, DWORD dwFlags, CWnd* pParentWnd)
{
	...
	// BUG: if last is \\, selection is not used (NT only?)
	m_strInitialFolderName.TrimLeft();
	int nLen = m_strInitialFolderName.GetLength();
	if (nLen > 3 && m_strInitialFolderName[nLen - 1]=='\\')
	{
		m_strInitialFolderName.GetBuffer(nLen);
		m_strInitialFolderName.ReleaseBuffer(nLen-1);
	}
	// verify that folder exists
	while (!m_strInitialFolderName.IsEmpty())
	{
		DWORD dwA = GetFileAttributes(m_strInitialFolderName);
		if ((dwA & FILE_ATTRIBUTE_DIRECTORY) && dwA!=(DWORD)-1)
			break;
		int nPos = m_strInitialFolderName.ReverseFind('\\');
		if (nPos<0)
			break;
		m_strInitialFolderName.GetBuffer(nPos);
		m_strInitialFolderName.ReleaseBuffer(nPos);
	}

	memset(&m_bi, '\0', sizeof(BROWSEINFO));

    pParentWnd = CWnd::GetSafeOwner(pParentWnd);
	...
	m_bi.ulFlags = dwFlags | BIF_STATUSTEXT | 0x040/*BIF_NEWDIALOGSTYLE*/;
...
void	CFolderDialog::SetTitle(LPCTSTR sTitle)
{
	m_bi.lpszTitle = sTitle;
}
