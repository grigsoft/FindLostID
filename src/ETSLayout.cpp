...
void ETSLayoutMgr::EraseBkgnd(CDC* pDC)
{
	...
	if( hBrBack == 0 )
	{
		hBrBack = (HBRUSH)GetWnd()->SendMessage(WM_CTLCOLORDLG, (WPARAM)pDC->m_hDC, (LPARAM)GetWnd()->m_hWnd);
		if (!hBrBack)
...
int ETSLayoutMgr::Pane::getMaxConstrainHorz()
{
	if(m_Orientation == HORIZONTAL) {
		...
		return (int)((nMaxConstr == -1) ? -1 : nMaxConstr + (m_paneItems.GetUpperBound()*m_sizeBorder) + 2*m_sizeExtraBorder);
	}
...
#if _MSC_VER >= 1400
LRESULT ETSLayoutDialog::OnNcHitTest( CPoint  pt)
#else
UINT ETSLayoutDialog::OnNcHitTest( CPoint  pt)
#endif
{
	LRESULT ht = Default();
	if (ht == HTCLIENT && !m_bFreezeLayout)
	{
		CRect rc;
		GetWindowRect( rc );
		rc.left = rc.right-15;
...
void ETSLayoutDialog::OnPaint()
{
	CPaintDC dc(this); // device context for painting
	if (m_bGripper && !m_bFreezeLayout)
	{
		CRect rc;
		GetClientRect(rc);
...
{
	if(m_RootPane.IsValid() && !m_bFreezeLayout) {
...
void ETSLayoutMgr::prepareRootBorders(CRect& rcClient)
{
